...
- [ ] IAP — on-device verification
  ...
  - [ ] On-device verification per `docs/store-integration.md` §4 remainder: a real test **purchase** + restore on a wiped local key (needs a Google account with test-card billing on the device — external, see `docs/blockers.md`).
