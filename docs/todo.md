...
## Adjust (2026-09-02 iteration)

- hide the custom keyboard option for now — DONE: the 🔢/⌨️ toggle is removed from `AnswerInput` and `MinesOfDoom` passes `useKeypad={false}`. The `NumericKeypad` wiring and the `onScreenKeypad` storage key stay for a future revival (revival steps in a comment in `AnswerInput.tsx`).
- pixel cave background should show a clear trajectory down the middle showing path of mining — DONE: the two middle tiles of every cave strip are always dug out (no rock/gems/eggs) with a dark wall edge on each flanking tile, so the cave reads as one vertical shaft (`CAVE_PATH_TILES` + `pathEdgeColor` in `src/utils/graphics/caveTiles.ts`; pinned by tests).
- add random easter eggs such as trapped princesses, spiders, etc. in pixel cave background outside of the center path of mining — DONE: ~15% of strips spawn a spider, a trapped princess, or a treasure chest in the rock outside the path (`eggForStrip` + `drawEgg` in `caveTiles.ts`). Deterministic per (tier, strip) — same texture-cycle discipline as the strips — and independent of the cave theme, so switching themes never relocates an egg. Covered by tests.
- add a pickaxe wind up animation during holding and pickaxe swing when released — DONE: `Miner` gained a `windingUp` prop — the pickaxe pulls back 55° while the player holds the cave (180ms in / 100ms out, native-driver) and the release snaps it forward into the normal per-pickaxe swing; a too-short hold just retracts. Driven from `MiningCanvas`' responder start/release, so it costs one re-render per press, never per tick.
- fix android build "unable to load script. Make sure you're either running Metro..." (jniLoadScriptFromAssets / CatalystInstanceImpl) — INVESTIGATED, no code bug: that message is React Native's standard launch error when the installed build can't fetch its JS bundle from a running Metro. Workflow fixes: (1) for a runnable debug build use `npx expo run:android` — it starts Metro itself, builds, and installs, so the bundle is always reachable (manual start-then-run can miss each other on port/host; the new VS Code tasks do the pairing if preferred); (2) an APK from `react-native build-android --mode=release` is a release build — it never talks to Metro, so if that APK shows the error, the gradle build didn't bundle the JS into the app's assets — for release artifacts use EAS Build or locally `npx expo run:android --variant release`, which always bundles the JS. Deferred: on-device verification (blocked, see item 11).
