...
- [ ] Fix ReferenceError: Property 'describe' doesn't exist, js engine: hermes on Android (not reproducible in web build; suspected expo-router/native-stack interaction — with the crash nets + context trail above, the next occurrence lands in Settings → "Recent errors (debug)" with a copyable full stack, the layer that caught it, AND the session event trail/state at crash time; remaining: reproduce on device and read the trace)
...
- [ ] Fix ReferenceError: Property 'describe' doesn't exist, js engine: hermes on Android (not reproducible in web build; suspected expo-router/native-stack interaction — with both crash nets above, the next occurrence lands in Settings → “Recent errors (debug)” with a copyable full stack AND the layer that caught it, even in release builds; remaining: reproduce on device and read the trace)
...
- [ ] Add the real Google/Apple store SDK integrations behind the `IapProvider` interface (Google Play Billing / StoreKit, or RevenueCat for receipt validation) — provider swap is one line in `MinesOfDoom.tsx`
