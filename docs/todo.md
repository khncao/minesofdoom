...
- [ ] Add the real Google/Apple store SDK integrations behind the `IapProvider` interface (Google Play Billing / StoreKit, or RevenueCat for receipt validation) — provider swap is one line in `MinesOfDoom.tsx`; also add the cosmetic IAP packs (still earnable in-game per §5) once the store round-trip exists)
