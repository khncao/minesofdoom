...
- [ ] Optional login (anonymous device default)
  ...
  - [ ] **External (Android):** the Google sign-in sheet needs its app-side credential before it can mint a real idToken — a Google Cloud OAuth client (android package `com.minus4kelvin.minesofdoom` + the upload-key SHA-1). Until then the button fails closed to the single inline error (honest — never a faked sign-in). The Apple sign-in half stays out of this file — tracked in `docs/backlog.md`.
