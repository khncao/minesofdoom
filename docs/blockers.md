...
## Web Google sign-in (GSI) fails on the deployed web build — **blocked on a Google Cloud Console change (2026-09-08)**

The Cloudflare Pages migration (commits `d55447e`/`9e237c5`) moved the
web app's ORIGIN from `https://khncao.github.io` to
`https://minesofdoom.pages.dev`. The in-repo half of the fallout is fixed
and verified (below); the OAuth2 half is a Google Console change — there
is no API/CLI for OAuth client authorized origins, so a human must click
it.

**Root cause (web "failed to sign in", OAuth2 leg):** the web Google
button mints the idToken through Google Identity Services
(`mintGoogleIdTokenWeb`, `signinSdks.ts`) with the Web-application client
`94426274846-7vsqc2habc84b0upion6clsdnl5cqj1f…` (`GOOGLE_WEB_CLIENT_ID`).
GSI only runs from origins listed in that client's **Authorized
JavaScript origins** — the list still reflects the pre-migration origin(s)
and does not include `https://minesofdoom.pages.dev`, so the token-client
popup flow errors before any idToken is minted (the settings UI's single
inline "failed to sign in" — `SignInCancelledError` is NOT this: a
cancel stays silent, this is the plain-Error branch). The client→server
half is not the suspect: the deployed Pocketbase answers CORS with
`access-control-allow-origin: *` (probed live 2026-09-06) and the
...
1. Google Cloud Console → project **minesofdoom** → APIs & Services →
   ...
   Android client).
2. *Edit client* → **Authorized JavaScript origins** → add
   `https://minesofdoom.pages.dev` (keep the old origin(s) until the
   pages.dev leg is verified, then optionally drop them).
   No redirect URI is needed: the GSI token-client flow is
   popup-and-token, no server redirect.
3. Verify in a real browser on
   `https://minesofdoom.pages.dev`: settings → Google button → consent
   popup → signed-in branch (email/password + Google coexist on one
   account, same as the 2026-09-05 native pass).
4. While in the console: confirm the **same client** id is what the
   sidecar's `GOOGLE_CLIENT_ID` env pins (`aud` check) — it is
   (`94426274846-7vsqc2habc84b0upion6clsdnl5cqj1f…`), unchanged by this.

**Fixed + verified in the same migration fallout (2026-09-08, no
console leg):** the sidecar CORS failure
(`POST /stripe/checkout` from origin `https://minesofdoom.pages.dev`
blocked — the exact `docs/todo.md` error). The sidecar allow-lists ONE
origin: the origin of `MDOOM_WEB_BASE_URL` (VPS `~/docker/pocketbase/.env`,
never in the repo), which still held the old
`https://khncao.github.io/minesofdoom`. Updated it to
`https://minesofdoom.pages.dev` and recreated the sidecar container
(`docker compose up -d sidecar` — `restart` does NOT re-read `.env`).
Verified live: the preflight from the new origin now gets `204` +
`access-control-allow-origin: https://minesofdoom.pages.dev` (old origins
still refused — fail closed), a real POST carries the CORS headers and
reaches the route, and the step-6 no-cost E2E
(`scripts/stripe/checkoutTest.mjs`) PASSED post-migration (session
`cs_test_a1c3gZvf…`, device `mdoom-step6-mtt2frc5` — return navigation
landed on `https://minesofdoom.pages.dev/?iap=success…` and asserted as
the app via its persistent "hold to mine" canvas caption, session
`complete`/`paid`, webhook leg minted, redirect leg HTTP 200, exactly 1
entitlement row, idempotent). The probe itself had a pre-migration blind
spot — its success/cancel URLs pointed at the Pocketbase public domain,
so its "return navigation" never landed on the app; it now points at
`WEB_BASE` (the app origin) and asserts the returned document is the app
by that caption (NOT `document.title` — the RN-Web runtime clears it
after the static load), and closes the page before the browser so a
still-loading return navigation can't crash node with exit 1 after the
PASS printout.
