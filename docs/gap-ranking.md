...
25. ~~**`account:me-network-wipes-token`**~~ (pass 41, F41.2) — **FIXED 2026-09-16 (pass 60)**: `AuthProvider.me()` is now tri-state (`AuthMeResult` — `account` / `dead` / `unknown`); only an explicit 401 is `dead` and only `dead` clears the stored token in `useAccount`'s mount restore, while transport failure / other non-2xx / malformed-200 (F41.5 folded — the 200-with-bad-body case) / unconfigured provider / a rejecting `me()` all map to `unknown` = token kept, stay anonymous for this run, retry next launch; the dev-sim map is the server of record there, so an absent token is genuinely `dead`. Nets: 5xx → unknown, malformed 200 → unknown (`auth.test.ts`), unknown and rejecting `me()` keep the stored token (`useAccount.test.ts`). The original finding: a real bug on
...
26. ~~**`compliance:policy-child-directed-mismatch`**~~ (pass 45, F45.1) — **FIXED 2026-09-16 (pass 60)**: the "Children" section rewritten to the recorded S6 position (teen+ 13+, not child-directed, under-13 deletion promise kept) in `legal.ts` v2.1, the published `public/privacy-policy.html` regenerated by `legalDocs.test.ts`, and the sibling net landed — `legal.test.ts` now asserts the policy's claim against `storeConfig.adMob.tagForChildDirectedTreatment`, so a future flag flip can't silently falsify the prose again. The original finding: the
    ...
    falsify it again.
27. ~~**`identity:device-id-fork`**~~ (pass 49, F49.1) — **FIXED 2026-09-16 (pass 60)**: `getIapDeviceId()` is now memoized + single-flight — concurrent callers share one in-flight promise (same class as the crashLog persistence chain), a resolved id serves from the memo, a failed read resets the slot so the next call retries; nets: concurrent fresh-install calls resolve the same id with exactly one mint + one write, memoization survives storage loss, a pre-seeded id is never re-minted. The original finding: a real bug under the
...
**F41.2 — `account:me-network-wipes-token` (Tier 1, #25 — real bug — FIXED in
pass 60, 2026-09-16: `me()` is tri-state `AuthMeResult` (account/dead/unknown),
only an explicit 401 is `dead`, only `dead` clears the stored token;
transport/other-non-2xx/malformed-200 (F41.5 folded)/rejects are `unknown`
= token kept; nets in `auth.test.ts` + `useAccount.test.ts`).**
...
F41.2 — folded into the pass 60 F41.2 fix: the malformed-200 branch
maps to `unknown`, netted in `auth.test.ts`).** Third of the three `me()` `null`s, named for the fix: a 200
...
+ **F45.1 `compliance:policy-child-directed-mismatch` (Tier 1 — new #26 —
  FIXED in pass 60, 2026-09-16: policy rewritten to the S6 13+ /
  not-child-directed position, `legal.ts` v2.1 + HTML regenerated + the
  policy-vs-config consistency net landed in `legal.test.ts`).**
  The "Children" section of the privacy policy asserted advertising is
...
path — FIXED in pass 60, 2026-09-16: memo + single-flight in-flight
promise, concurrent callers resolve one id, nets in
`iapDeviceId.test.ts`):** `getIapDeviceId()` (`iapDeviceId.ts`) is a read-generate-write
...
### The juice / game-feel feedback layer (pass 51 — what a mine feels like: the wave model, the haptic/sound/particle/shake channels, and the reduce-motion gate, written 2026-09-15)

Audited the game-feel layer end to end for the first time — pass 22 audited sound, pass 31 what's seen (art), pass 20 the cost of a second, but nobody had inventoried the *feedback channels themselves*: who fires what, what scales, and what the two player-facing controls (`haptics`, `reduceEffects`) actually reach. Surface: one wave model (`juice.ts` — waves + floating-text size, one per decimal digit of the gain, capped at 5), one schedule (`useJuiceWaves`, wave i at i×130 ms, all timers cleared on unmount), the haptic channel (`haptics.ts` + `useHaptics`, patterns + 50 ms throttle + settings toggle), the sound channel (the `play()` call sites at each reward moment), and the motion consumers: Miner swing + idle bob, `DebrisParticles`, `BlockBreak`, the combo counter flash, the gem-pocket pulse, the answer-field shake, the floating "+N" layer, and the SavePill/LoadingScreen pulses.

**Verdict: the layer is well-designed and almost fully gated — the wave model is the design win (one `getJuiceWaves(gain)` drives swing waves, debris repeats, floating-text size, and tap-haptic duration, so the channels can't drift apart), and the reduce-motion principle ("suppress the decorative repeats, keep the action's own hit") is applied to 7 of 9 motion channels — the two exceptions are the two highest-frequency motion sites in the game (F51.1).**

+ **F51.1 `accessibility:juice-motion-bypass` (Tier 2):** the OR'd reduce-motion decision (`useAccessibilityReduceMotion` — manual `reduceEffects` toggle OR web `prefers-reduced-motion`; the module doc claims it turns the decorative effects off) gates the combo flash (`useCombo`), the miner idle bob, the gem-pocket pulse, the debris burst, the SavePill and LoadingScreen pulses, and both juice-wave collapses (tap path in `useMineTaps`, answer path at the call site — each with its own test). It does **not** gate the two motion channels that fire most often: (a) `useShakeInput.shake()` — the answer field's ±10 px / 400 ms translateX, called unconditionally from *both* wrong-answer paths (`onIncorrect` and `onSoftIncorrect`), i.e. the one effect that fires on *failure*, and the most vestibular-salient animation in the game; and (b) the floating "+N" layer — `FloatingTextLayer`'s 900 ms float-up on **every** gain (tap, answer, pocket, gem vein), which takes `_props` and ignores them, so no gate exists to flip. The sibling `BlockBreak` pop is *not* a bypass: it's the "action's own hit" the collapse rule deliberately keeps (waves go to 1, the block still breaks). Fix is two small gates: no-op `shake()` under `reduceMotion` (pass the flag into `useShakeInput` or gate at the two call sites), and a `reduceMotion` prop on `FloatingTextLayer` that swaps the float-up for a fade-in-place (opacity only, same info). Nets: the existing `useMineTaps`/`useCombo` reduce-motion test pattern extended to the two new sites. Tier 2 — real accessibility-symmetry fix, cheap, no behavior change for players who didn't ask.
+ **F51.2 `juice:wave-interval-net` (Tier 3):** `JUICE_WAVE_INTERVAL_MS` (130 ms) is load-bearing by comment — it "must clear the internal throttles" of its three effect targets, whose throttle constants are local (Miner `MIN_PICKAXE_INTERVAL` = 100, `DebrisParticles` and `BlockBreak` `MIN_TRIGGER_INTERVAL` = 80 each) — and **no test pins the invariant**: the three throttle constants aren't exported, and `juice.test.ts` never imports the interval. Raise any target's throttle past 130 and wave 2+ is silently dropped (the wave fires 130 ms after wave 1, inside the throttle window) with no gate failing. Fix: export the three throttle constants and add one line to `juice.test.ts` asserting `JUICE_WAVE_INTERVAL_MS > max(all three)`.
+ **F51.3 `haptics:dead-export` (Tier 3, hygiene):** `getTapHapticDuration` is exported and carries 5 test assertions, but no production code calls it — the live tap path is `getHapticPattern("tap", intensity)`, a parallel implementation of the same expression over the same `TAP_BASE_MS`/`TAP_STEP_MS`/`TAP_MAX_MS` constants. The test suite currently gives a false sense of net over a dead function. Fix: delete the export (and its assertions), or route the tap branch of `getHapticPattern` through it so the tested code is the running code.

Recorded, not defects: **the feedback is bounded at every site** — `MAX_JUICE_WAVES` = 5, `DebrisParticles` `MAX_PARTICLES` = 12 (drop-new-est), `BlockBreak` `MAX_BLOCKS` = 5, `FloatingTextLayer` `MAX_ITEMS` = 16 (oldest keep running out, new dropped), the 50 ms haptic throttle (one vibration per burst — an achievement landing on the same frame as its answer collapses to one buzz by design), and the 20 Hz tap flush — the "Excessive number of pending callbacks" class is netted by construction; `useJuiceWaves` cancels all wave timers on unmount. **Wave-timer stacking under tap spam is bounded by tap rate** (each `run` schedules ≤ 4 timers, each ≤ 4×130 ms out; F47.1 already records the same fire-and-forget family for the per-batch removals). **Haptics not gated by `reduceEffects` is correct** — different modality, own settings toggle; on web the `Vibration` call is a platform no-op (documented in `useHaptics`). **The sound side of juice scaling is Tier 1 #6** (`reward-sfx-set` + `sfx-gain-scaling` — "sound is the one juice channel that ignores juiceWaves"), not re-ranked here. **Non-core reward moments ride toasts only** (ad reward, daily bonus, weekly — no haptic/sound at the grant site itself beyond what `useAdRewards`/`useDailyBonus` already play), consistent with the "toasts carry meta events" split. **The `dailyEquation` double-haptic** (a `success` then a `tap` in the same submit) drops the second under the 50 ms throttle — the *success* beat wins, which reads correctly for a solved day. **OnboardingOverlay is deliberately static** (works under reduce-motion by construction), and the shared animation clock (`animationClock.ts`, the miner-bob phase source) is unit-tested.

Method: the layer's shape is the channel inventory — every `play(` / `haptic(` / `spawn(` / `trigger(` / `shake(` / `flash(` call site in `MinesOfDoom.tsx` + `useMineTaps` read in full (8 haptic sites, the two juice-wave `run` sites, the two `shake` sites, pocket/daily/answer reward moments), each motion consumer's internal gate read (`useCombo` flash early-return, `Miner` bob `useMemo` null, `MiningCanvas` pocket-pulse loop stop, `DebrisParticles` trigger ref-check, `SavePill`/`LoadingScreen` pulse gates, `AnswerInput` transform binding, `FloatingTextLayer`'s ignored props), the two settings rows in `SettingsPanel` (haptics + reduceEffects, both i18n'd en+es), and the cap/throttle constants read at each effect target. The reduce-motion coverage question is answered by grep: `reduceMotion` appears in the 7 gated modules and in `useMineTaps.test.ts` / `useCombo.test.ts` (the collapse and static-flash nets) and in *neither* `useShakeInput` nor `FloatingTextLayer` (no suites exist for either — consistent with the repo's pure-logic-only test convention, pass 42). The net question (F51.2) is the `juice.test.ts` import list plus a grep for the throttle constants (local, unexported). Pass 51 made no code change; F51.1–F51.3 are ranked only, pending greenlit.

### Onboarding and first-session flow (pass 52 — the one-time tutorial and the save-code round trip: where "start playing" and "resume playing" diverge, written 2026-09-15)

Audited the player-entry flow end to end for the first time — pass 20/21/26/28 audited the persistence and i18n around it, but nobody had walked the *journey* from cold boot to "player understands the game", or the second entry point (the save-code import in `MenuPanel` → `SaveTab`). Surface: `OnboardingOverlay` (4 steps — three loop tips, `TIP_KEYS`, plus the first-time setup step, `TOTAL_STEPS = TIP_KEYS.length + 1`), its flag (`onboardingDone`, a `useLocalStorage<boolean>` key *separate* from the save), the single dismiss path, the two gates the flag feeds (the render gate and the idle-reminder gate), and the save-code export/import pair in `useGameEngine` (`exportSaveCode`/`importSaveCode`) with the settings ride-along applied by the `MinesOfDoom` handler.

**Verdict: the flow is honest and the copy is accurate (every claim in the three tips checks out against the engine — combo is broken by wrong answers and cave holds, not by a timer; tip 3 points at the 🎯 goals column; the setup step deliberately reuses the `settings.opName.*` / `settings.onScreenKeypad` keys so the player recognizes them later in the menu); the gaps are at the two seams where "first session" and "resumed session" diverge:**

+ **F52.1 `onboarding:save-code-import-replays-tutorial` (Tier 2 — FIXED in pass 59, 2026-09-16):** `onboardingDone` is not part of the save — the `SaveCodePayload` is `SaveData & {settings?, equationSettings?}` (saveCode.ts), so the flag never travels in a save code, and the import path never touches it (the engine's `importSaveCode` is settings-agnostic by contract and the `handleImportSaveCode` handler only applies the settings ride-along). Fix implemented as specified: additive optional `onboardingDone?: boolean` on `SaveCodeSettings` (emitted only when `true`, so legacy-shaped codes stay byte-identical), export threads the live flag, import applies it when `true` (absent → keep local); round-trip net in `saveCode.test.ts`. The failure direction is exactly the feature's point: save codes exist to "back up or transfer a save without a cloud account" (saveCode.ts doc), so a returning player who restores their veteran save on a new device via save code lands past depth 0 with the 4-step tutorial replayed on top. The reverse direction is harmless (a new player who imports a modest friend save *needs* the tutorial). Fix shape: an **additive optional** `onboardingDone?: boolean` on `SaveCodeSettings` — the same round-trip pattern the two existing ride-along fields use (absent on every legacy code, parsed additively, interops both ways) — written on export from the flag, and applied in `handleImportSaveCode` via the existing flag setter when `true` (absent → keep local state, i.e. today's behavior). Prestige is deliberately *not* a seam: `sinkNewShaft` never touches the flag, and a veteran sinking a new shaft has already learned the loop.
+ **F52.2 `onboarding:no-replay-path` (Tier 3 — FIXED in pass 59, 2026-09-16):** once dismissed, the tutorial cannot be re-shown — the flag had **exactly one write site** (the single `onDismiss` closure at MinesOfDoom L1815, reached from both the Skip button and the final "Start mining!" button) and **no clear site anywhere** (grep-verified: no replay row in any settings component; the Reset/prestige path does not clear it either). Fix implemented as specified: a "Replay the tutorial" button in the Save tab (SaveTab → MenuPanel → MinesOfDoom `handleReplayTutorial` = `setOnboardingDone(false)`) + i18n in `en.ts`/`es.ts`; the existing render gate re-opens the overlay on the next frame. A player who skipped and then got lost has to re-read the menu blind. The overlay is a static tip sheet (no state machine, no progress tracking), so replaying it mid-run is safe. Fix shape: a settings row that sets the flag to `false` (one call to the existing setter + i18n in both `en.ts`/`es.ts`), which re-opens the overlay on the next render — the existing `onboardingLoading`/`onboardingDone` render gate does the rest.

Recorded, not defects: **the copy is accurate where it matters** — tip 2 says the combo is broken by "wrong answers and cave holds" (correct per the engine; the idle *reminder* toast is a separate feature and the tip makes no timer claim), and tip 3's "Check the 🎯 goals" points at the real goals column; **the loading gate is deliberate anti-flash** — the overlay is hidden while the flag loads (`!onboardingLoading` in the render gate) and the game screen is `focusable={onboardingLoading || onboardingDone === true}` (L1620) so input can't reach the canvas before it's known whether the overlay will appear; **the idle-reminder gate is correct** (`useIdleReminder` is `enabled` only when `settingsData.idleReminder && !onboardingLoading && onboardingDone === true`, L824–832 — a slowly-reading first-time player is not idle, and no toast lands on the full-screen overlay); **the engine's `void settings` in `importSaveCode` is the layering contract, not a leak** — the engine stays settings-agnostic and the caller applies the ride-along (same pattern as `restoreFromBlob`'s `onSettings` callback on the cloud path); **the setup step's key reuse is by design** (documented in en.ts: a player who set operators here recognizes the same rows in Settings); **a11y labels exist for every control** (`onboarding.a11ySkip/Next/Start`).

Method: the layer's shape is the journey, so the audit walked it in order — `OnboardingOverlay.tsx` read in full (the 4-step structure, the single `onDismiss` reached from both the Skip button at L135–136 and the final step's start button at L116–117, the setup step's props: `equationSettings`/`onEquationSettingsChange`/`onScreenKeypad`/`onKeypadChange`), the flag's definition and its two read gates in `MinesOfDoom.tsx` (L182–183 definition; L824–832 idle-reminder gate; L1620 focusable gate; L1809–1816 render gate + the one `setOnboardingDone(true)` write), the tip copy in `en.ts` L172–185 checked claim-by-claim against the engine (combo-break rules, the goals column), and the save-code pair in `useGameEngine.ts` (`exportSaveCode` L963–969, `importSaveCode` L975–1004) plus the handler in `MinesOfDoom.tsx` L1064–1090 (ride-along application, toasts, the crash-ring note) and the payload shape in `saveCode.ts` (`SaveCodeSettings`/`SaveCodePayload`, the additive-parse precedent). The two seam questions are grep answers: "who else writes `onboardingDone`" (one site — so no replay path, F52.2) and "what is in the export payload" (the `SaveData` fields + two additive settings objects — no flag, F52.1). The UI reach is one hop from the settings root (`MenuPanel` → `SaveTab`, props at L52–53/L136–137 and L25–26). Pass 52 made no code change; F52.1–F52.2 are ranked only, pending greenlit.

### The input / answer-entry layer (pass 53 — every path player intent takes to the game: the OS keyboard, the keypad, and the cave, written 2026-09-15)

Audited the input layer end to end for the first time — pass 20 covered the *cost* of a second of input, pass 47 the timer lifecycles inside it, pass 51 the feedback it fires, but nobody had inventoried the *entry paths themselves*: where player intent enters the game, how it is parsed, and what a malformed or empty intent costs. Surface: the two answer paths (the OS-keyboard `TextInput` in `AnswerInput`, and the `NumericKeypad` strip — both funnelling into `useEquations.handleSubmit`), the cave's hold-to-mine responder in `MiningCanvas`, the `IntegerInput` numeric-setting rows, and the parse boundaries (`parseFloat` in `handleSubmit`, `parseInt` in `IntegerInput`, the generator's own clamps).

**Verdict: the layer is deliberately over-built for stability (stable callbacks so the memoized input skips the 1Hz tick, a responder-system canvas instead of Pressable, the OS keyboard fully avoidable via the keypad) — and the one real gap is that an *empty* answer is parsed as a *wrong* one:**

+ **F53.1 `input:empty-submit-penalized` (Tier 2 — FIXED in pass 64, 2026-09-16: `handleSubmit` now early-returns on `textInput.trim() === ""` before the parse, covering the soft-mode branch too, so an empty or whitespace-only submit (Return in an empty box, keypad `=` with nothing typed) fires no callback, pays no penalty, and rolls no equation; netted in `useEquations.test.ts` for both the basics and soft-incorrect suites):** `useEquations.handleSubmit` has no empty-input guard (historical finding text below) — `Number.parseFloat("")` → `NaN`, and `approxeq(NaN, answer)` is `Math.abs(NaN - x) <= 0.01` → `false`, so an empty submit falls through to the **full** `onIncorrect` path (MinesOfDoom L929–931: `play("stone", 150)` + `shake()` + `haptic("error")` + combo reset with resistance + the `comboLost`/`comboDropped` toast). Both entry points reach it with zero typing: the OS-keyboard path (`onSubmitEditing`, AnswerInput L110–113 — Return in an empty box) and the keypad path (`NumericKeypad`'s `=` key, wired as `onSubmit={handleSubmitActivity}` at MinesOfDoom L1728 — the cheapest misclick on the game's primary UI). The player gets penalized for answering nothing. (In the daily-equation soft mode the same empty submit hits `onSoftIncorrect` instead — no combo loss, but still the full error sound + shake + haptic for typing nothing.) Fix: one early return in `handleSubmit` — `if (textInput.trim() === "") return;` before the parse (covering all three branches, soft and normal) — plus the net `useEquations.test.ts` lacks: an empty submit fires neither `onIncorrect` nor `onSoftIncorrect` and rolls no new equation.
+ **F53.2 `i18n:cave-hold-label-hardcoded` (Tier 3 — FIXED in pass 57, 2026-09-16: new `ui.holdToMineHint` content key in en/es wired into `MiningCanvas` (the same finding from the rendering side, F57.1):** the cave's visual instruction (historical finding text below) `<Text>hold to mine</Text>` (MiningCanvas L324) is a hardcoded English string — the one player-facing label in the input layer that is not routed through `content()`, while its *accessibility twin* is i18n'd (`a11y.holdToMine`, present in both `en.ts` L548 and `es.ts` L558). The AGENTS convention (player-facing strings live in `en.ts` + `es.ts`, type-checked for parity) is broken exactly where the two can silently drift: a Spanish player gets "Mantén pulsado para minar" read by the screen reader and "hold to mine" rendered on screen. Fix: add a content key (e.g. `cave.holdToMine`) to both files and render `t(...)` — one line each.

Recorded, not defects: **the generator clamps hostile settings at the boundary** — `getRandomEquation` floors `minNumber` at 0 and `maxNumber` at 2 (`Math.max(2, Math.floor(prefs.maxNumber ?? 12))`, equations L291) and falls back to plain multiply when no op fits, so the *unclamped* `IntegerInput` (a `parseInt` with no range check — a hardware keyboard on web can type `-5` or `999999999999` into the maxNumber row, and the save-code settings ride-along carries `equationSettings` unclamped too) is harmless at the parse boundary: the stored value is odd, the generated equation is not. **`IntegerInput`'s loose parsing is bounded and cosmetic** — empty/NaN → `clear()` + keep the last valid value; `"3.5"` applies `3` while the field still displays `"3.5"` (uncontrolled `defaultValue`, no remount), and the field likewise does not refresh when an imported save-code ride-along changes the setting — display drift only, callers clamp (autosave `5..600`). **The keypad's digit cap is the shared constant** (`MAX_ANSWER_LENGTH` = 12, exported by AnswerInput and applied in `handleKeypadDigit`), its bottom row is four deliberate `0` keys, ⌫'s `onLongPress` = clear-all (with an a11y hint), and every key carries an a11y label + testID. **The cave's quick-tap no-op is documented design** (MINE_HOLD_MS = 300 — quick taps used to reset the combo accidentally while players answered; the gem pocket deliberately keeps quick-tap collection because "an accidental tap can only ever help"). **`onSubmitEditing`'s imperative `clear()` is belt-and-braces** — all three `handleSubmit` branches already do `setTextInput("")`, and RN's `clear()` is a no-op on web, so the controlled-prop clear is what actually covers the web path. **There is no separate physical-keyboard handler on web** — RNW's `TextInput` is a DOM input; `inputMode="numeric"` is a hint, not a filter (a web hardware keyboard can type `"12a"` → `parseFloat` → 12; loose, but the player still had to know the answer, and the keypad path is digits-only by construction). **The stability architecture is the layer's design win** — `handleSubmit` reads through `latestRef` so the callback identity never changes, which is what lets `AnswerInput`/`NumericKeypad` skip re-rendering on the 1Hz tick and the 20 Hz tap flush (the "lag while tapping" cost, worst on web). The 20 Hz tap flush and the responder-system canvas itself are pass 20/47/51 territory, not re-audited.

Method: the layer's shape is the path inventory, so each entry point was read in full — `AnswerInput.tsx` (the AvoidingView web/no-op split, the `useKeypad` display-box swap, the `focusable`/`autoFocus` contract and its e2e-v8 history, `onSubmitEditing`), `NumericKeypad.tsx` (the four-column layout, the flex-shrink tap-target floor, the a11y/testID surface), `useMineTaps.ts` (the 20 Hz flush — pass 51's territory, read only for the entry-point seam), `useEquations.ts` (the `latestRef` stability pattern, the three-branch submit, the empty-parse hole), `MiningCanvas.tsx` (the responder negotiation, the 300 ms hold gate, the pocket's quick-tap exemption, the hardcoded label), `IntegerInput.tsx` (the `parseInt` boundary), and the two parse boundaries they feed (`approxeq` at equations L103, the generator's clamps at L291). The penalty claim is the `onIncorrect` callback in `MinesOfDoom.tsx` L928–944 (sound/shake/haptic/resistant reset/toast) and the wiring at L1728 (keypad `onSubmit`) and L1613 (AnswerInput's `onSubmit={handleSubmitActivity}`), both via the `handleSubmitActivity` wrapper at L958; the i18n claim is the en/es grep (`a11y.holdToMine` present in both, the visual string in neither); the "is there a net" question is the `useEquations.test.ts` inventory (basics + soft-mode suites — no empty-submit case). Pass 53 made no code change; F53.1–F53.2 are ranked only, pending greenlit.

### The settings / preferences persistence layer (pass 55 — where the ten-and-ten player preferences live, how they're validated, and where they can rot, written 2026-09-15)

Audited the settings layer end to end — pass 21 covered the save blob, pass 37 the numeric ledger, pass 52 the save-code *import* path, but the settings stores themselves (the thing both of those ride on) had never been inventoried: which key holds what, who writes it, who reads it, and what validates it. Surface: two structured stores — `settings` (`SettingsData`, 10 fields: autosave, showAllPurchases, emojiArt, haptics, reduceEffects, idleReminder, music, soundVolume, musicVolume, notation) and `equationSettings` (`EquationSettings`, 11 fields: minNumber, maxNumber, hardMode, multiplySymbol + the seven operator toggles) — both owned by `useSettings` (per-key AsyncStorage, merge-over-defaults on load, per-change persistence, an explicit Save that re-writes + saves the game); four sibling `useLocalStorage` booleans that are also player-facing settings but live outside both stores (`mute`, `onScreenKeypad`, `onboardingDone`, `iapRealStore`); the consumer-side clamps (`clampSoundVolume`/`clampMusicVolume`/`clampNumberNotation` at the settings→consumer seams, the engine's autosave `5..600` clamp, the generator's min/max floors — pass 53's territory); the portability ride-along (save codes *and* cloud snapshots serialize both stores via `serializeSavePayload`, validated on the way in by `pickPartial` — type-correct per-field selection keyed off the defaults); and the i18n surface (121 `settings.*` keys, en/es parity type-checked, localization itself disabled — English only, the locale store pinned).

**Verdict: the layer is the most defensive of any settings surface in the repo — every field is consumed, every numeric setting is clamped at its consumer, the portability path is type-validated, and the load-vs-write race is guarded — and its two gaps are the *one* unvalidated seam (the local stores' own load merge) and the *one* untested net (the hook that owns the race guard):**

+ **F55.1 `settings:load-merge-unclamped` (Tier 3):** the two local settings stores load with a bare object merge — `setSettingsData({ ...defaultSettingsData, ...parsed })` (useSettings L92) after only `typeof parsed === "object"` (L88–90) — with **no per-field type check**, unlike every other parse boundary in the layer: the save blob itself (`buildSaveData` clamps + drops unknowns), the save-code ride-along (`pickPartial` picks only same-`typeof`-the-default fields), even the equation generator (floors min/max). A corrupt or hand-edited `settings` key can therefore carry wrong-typed values straight into live settings. The boolean fields are the exposed ones: `haptics: "off"` is a *truthy* string → `useHaptics("off")` passes the `if (!enabledRef.current)` guard → haptics ON (and the `Switch` renders ON too, since a truthy string reads as checked — the UI agrees with the corruption, so nothing surfaces it); `music: false` stored as `"false"` → music ON. The numeric fields degrade silently instead: `soundVolume: "80"` → `clampSoundVolume`'s `typeof === "number"` check fails → default 100 (a player who hand-set 80 hears the jump). No repo writer can produce this — both writers in the file write complete, typed objects, and per-change persistence means the stored value is always the last UI state — so the only routes in are a hand-edited localStorage/MMKV entry or a future writer bug; that's why it's Tier 3, not higher. Fix: validate the load merge with the same per-field selection the ride-along already uses — export `pickPartial` (or move it to a shared module) and load `{ ...defaults, ...pickPartial(defaults, parsed) ?? {} }` for both stores; one shared helper, two call sites.
+ **F55.2 `settings:hook-untested` (Tier 3 — FIXED in pass 64, 2026-09-16: `useSettings.test.ts` landed in the house pattern — 9 nets: fresh-start defaults, stored-load round-trip, partial-save merge over defaults, corrupt-JSON fallback, functional-update composition, per-change full-object persistence, the load-vs-update race shield (a session update survives the lazy load resolving after it), fire-and-forget write failure, and `handleSaveSettings` (game save + both-store rewrite + toast)):** `useSettings` owns the layer's only race (historical finding text below) — a load that starts at mount must not clobber a change made before it resolves (the `settingsTouchedRef`/`equationTouchedRef` guards at L55/L88/L99/L105), plus the per-change persistence and the merge-over-defaults forward-compat ("an update adds a field, old saves get the default"). **None of it has a unit test** — `__test__/` has hook suites for every sibling with comparable state logic (`useEquations`, `useCloudSave`, `useCombo`, `useMineTaps`, `useAccount`), but no `useSettings.test.ts`, so the guard is protected only by the fact that it currently works. The test is mechanical in the house pattern: seed the AsyncStorage mock with a stored value, render, flip a setting *before* the load's promise resolves (the touched shield), await, assert the store kept the flipped value and the persisted JSON is the flipped one; plus a merge-over-defaults case (stored object missing a field → default supplied) and a corrupt-JSON case (defaults survive).

Recorded, not defects: **`mute` and `onScreenKeypad` are the two player-facing settings that don't ride along** — they're `useLocalStorage` booleans outside both stores, so save-code *and* cloud restore transfer neither: a player restoring a cloud backup on a new device loses their mute and keypad choices (a deliberate "display preference, not a save" line — the hook comments say so — but it's a documented asymmetry of the settings-portability feature, and the natural fix if it ever bites is moving them into `SettingsData`, which the ride-along already carries wholesale). **The min/max inversion is a silent swap, not a bug** — `getRandomIntInRange` does `lo = Math.min(min, max)`, so a player who sets a min above the max (or a save code that does) gets the inverted range, playable and bounded; the generator's floors (pass 53's finding) make it non-hostile. **The hard-mode gate hides the switch, not the setting** — `hardModeUnlocked` (tier-5 complete) only controls the UI affordance; an imported ride-along with `hardMode: true` applies unconditionally, which is the consistent reading ("you imported *their* settings") since the same path already imports their operator mix and number ranges. **`minNumber` has no UI row** — only `maxNumber` is player-editable (the panel's single IntegerInput); `minNumber` exists at its 0 default or via save-code ride-along, which is exactly the pass 53 "unclamped but boundary-safe" profile. **The explicit Save button re-writes settings that are already persisted** — deliberate: the per-change write covers the setting, the Save button's job is the *game* save + the honest "Saved" toast, and the double-write is a no-op of identical bytes. **Localization is disabled by design** — `settings.language`'s row key exists in both tables but the i18n locale store is pinned to English (`resolveLanguagePreference` never flips it); the 121-key es table is carried type-checked in parallel, ready for the flip.

Method: the layer's shape is the store inventory, so it was read store-by-store — `game.ts` L85–180 (`SettingsData` + key constants) and L795–880 (`defaultSettingsData` + the three clamps), `equations.ts` L20–70 (`EquationSettings` + defaults) and the generator boundary (pass 53, re-verified), `useSettings.ts` in full (the load effect, the touched shields, per-change persistence, `handleSaveSettings`), the consumer seam in `MinesOfDoom.tsx` (the autosave ref at L191/L389, the notation push at L299, the sounds gate at L591–632, the haptics/reduce-motion/idle-reminder gates, the cloud-snapshot ride-along at L444–456, the import application at L411–425), `saveCode.ts` L223–241 (`pickPartial` + `parseSaveCodeSettings`), `restoreFromBlob` in `useGameEngine.ts` (the same clamping pipeline as the storage loader), the panel (`SettingsPanel.tsx` head + the volume step rows with their clamps, the SaveTab autosave row with its 5..600 clamp), the four sibling keys' declaration sites, and the i18n inventory (121 `settings.*` keys in en.ts, parity type-checked by the tables' shared key type). The corruption claim is the L88–90 merge read against the `useHaptics` enabled-guard (L21–27 — a truthy string passes) and the `clampSoundVolume` typeof-falls-to-default path; the "is there a net" question is the `__test__/` inventory (53 suites, no `useSettings.test.ts`; the clamp trio is netted in `game.test.ts`, the ride-along in `saveCode.test.ts`, the hook itself unnetted). Pass 55 made no code change; F55.1–F55.2 are Tier 3, ranked only, pending greenlit.

### The rewarded-ad economy layer (pass 56 — the reward rules, the claim lifecycle, and the monetization guardrail check, written 2026-09-16)

Audited the rewarded-ad economy end to end — plan §5.1's reward surface: what a completed ad grants, how the daily caps are metered, and how the claim lifecycle is wired through the provider abstraction. Pass 17 owned the offline-haul formulas (8 h cap, +2 h top-up slice) and pass 25 the engine's progression surface, so this pass took the *ad-specific* delta on top: the caps and eligibility in `ads.ts`, the claim state machine in `useAdRewards.ts`, the pending-offer refs in `useGameEngine.ts`, the combo-save window in `MinesOfDoom.tsx`, and — because the reward economy is where the non-negotiables live — a check of the monetization guardrails themselves (what money can and cannot buy).

**Verdict: the rule layer is clean and the monetization guardrails hold structurally.** The caps are enforced in pure code (`ads.ts`) and the provider can only ever trigger them: gem rolls 5 gems × 3/day, combo save 1/day with a 60 s restore window, fraud cap 10/day total, all reset on the local-day key; the meter is persisted *outside* the save on purpose (a shared/imported save must never import the sender's meter — and losing it can only cost free rewards). The claim lifecycle is single-flight (`claimingRef`, one ad at a time ever) with a latest-state re-check via `stateRef` (the same race class the daily-bonus guard covers), and the combo-save window race is already guarded: the expiry `setInterval` unmounts while a comboSave ad is mid-play (effect re-runs on `claiming`, cleanup clears the interval), so an offer the player tapped survives the ad; worst case is a one-tick-late restore, documented in the code. The guardrail check came out clean on both counts: **guardrail 1 (F2P viability) holds structurally** — the IAP catalog is cosmetics-only (pickaxe/outfit/skin/theme packs in the $0.99–$3.99 band, every blurb ending "Purely cosmetic"), so every power upgrade stays gem-earned and the free path reaches the same end-state by construction, not case-by-case enforcement; **guardrail 2 (rewarded-only) holds** — no interstitial/banner surface exists anywhere, the entry points are hidden end-to-end whenever `pickAdProvider` yields the no-op, and the dev-sim provider (the only way rewards exist before a real SDK) is `__DEV__`-gated and UI-labeled as a simulation. Balance: ad gem income is 15 gems/day against gem sinks of n⁴+1 for miner tiers and 10–25·(l+1)² for the upgrade lines — supplemental early (a 1–2 g upgrade in under a day of ads), never dominant mid-game (a 625–1 241 g miner is ~2 weeks of max ad income).

+ **F56.1 `ads:stale-offer-survives-save-replace` (Tier 3 — FIXED in pass 56, 2026-09-16):** the pending ad offers — `offlineDoubleRef` (the "watch to double" haul) and `offlineTopUpRef` (the +2 h slice beyond the 8 h cap) — are set at load and at import/restore, consumed at claim, but **never cleared when the save is replaced**: `resetGame` touches neither ref nor its UI state (the ad panel still shows the stale rows after a reset!), and `importSaveCode`/`restoreFromBlob` replace the top-up only when the imported save has one (`if (topUp > 0n)`) and never touch the double. So a discarded session's ≤10 h haul (8 h double + 2 h top-up) is claimable via a completed rewarded ad against an unrelated new save — e.g. reset, then watch the double row that is still there, or import a save and claim the pre-import double. One-shot per replacement event (the ref is consumed), small value (≤10 h of offline income), requires a deliberate reset/import. Fix (small, in `useGameEngine.ts`): clear both refs + their UI state in `resetGame`, and in `importSaveCode`/`restoreFromBlob` clear-then-conditionally-set (always assign, so a stale offer is never orphaned).

Recorded, not defects: **the "gem rolls" copy is accurate, not a naming rot** — the row reads "💎 Gem rolls — +{count} 💎 per watch" with count = the exact flat 5-gem grant, so the plan's "5 free gem rolls" landing as a flat grant is a copy-level non-issue. **The meter surviving a reset is the conservative direction** — `adRewards` (the fraud meter) is a separate key that neither reset nor import touches, so a reset can only *cost* the player their remaining free rewards of the day, never grant extras; that asymmetry is the safe side of the coin and matches the module's stated intent. **"Erase all data" covers the meter** — it wipes by `getAllKeys()`, so the key is removed without a registry to keep in sync (pass 55 territory). **The offline haul / 8 h cap / +2 h slice formulas are the pass 17 layer** — `computeOfflineMinerals`/`computeOfflineTopUpMinerals` cross-referenced only, not re-audited. **The import double/top-up asymmetry** (import offers a top-up but never a double, unlike a fresh load) is subsumed by F56.1 — the fix's clear-then-set covers both. **The dev-sim provider always resolving "rewarded"** (no "closed" path) is deliberate: it exists to exercise the full flow, and it is unreachable in production builds.

Method: `ads.ts` read whole (caps, eligibility, metering), `useAdRewards.ts` read whole (claim lifecycle), `useGameEngine.ts` for the load-time offer setup (L155–270), the two claim consumers (L425–470), and the save-replacement paths — `importSaveCode`/`restoreFromBlob`/`resetGame` (L960–1100) — plus the combo-save wiring and its expiry effect in `MinesOfDoom.tsx` (L655–700, L1150–1172). Guardrail check via `iaps.ts` (the pack catalog: `PACK_SPECS`, `packPriceLabel`'s $0.99–$3.99 band, the "Purely cosmetic" blurbs) and the gem cost curves in `game.ts` (L1323–1472). `pnpm jest src/mines_of_doom/__test__/ads.test.ts` (24/24 green before this pass — the rule layer's existing net). Docs-only pass: no source touched, so no gates run.

### The cosmetics & cave-theme rendering layer (pass 57 — data model, seeded look rolls, cave-tile bakes, canvas/Miner render)
...
### Pass 59 — F52.1/F52.2 implemented (onboarding flag in save codes + tutorial replay row)

Both pass 52 findings fixed as their spelled-out shapes prescribed (1221/1221 gates green):

+ **F58.→F52.1** (`onboarding:save-code-import-replays-tutorial`, Tier 2): additive optional `onboardingDone?: boolean` on `SaveCodeSettings` — emitted only when `true` (legacy codes byte-identical), parsed boolean-typed (corrupt values dropped), export threads the live flag from `handleExportSaveCode`, import applies it via the existing setter when `true` (absent → keep local state, the legacy behavior). Round-trip net in `saveCode.test.ts` (flag present → round-trips; absent → not even serialized; corrupt string → dropped).
+ **F52.2** (`onboarding:no-replay-path`, Tier 3): "Replay the tutorial" button in the Save tab (`settings.replayTutorial` i18n key, en/es) — `SaveTab` (new required `onReplayTutorial` prop) → `MenuPanel` (pass-through) → `MinesOfDoom.handleReplayTutorial` = `setOnboardingDone(false)`; the existing `onboardingDone !== true` render gate re-opens the overlay on the next frame. The overlay is a static tip sheet, so mid-run replay is safe.
+ Notes: the engine's `importSaveCode` stays settings-agnostic by contract — `onboardingDone` is another ride-along on `SaveCodePayload` (destructured + voided in the engine, applied by the caller), exactly the established pattern. Cloud save is unaffected (the flag is not in `SaveData`, so cloud blobs never carry it — a cloud restore replays the tutorial like before; only save codes now carry it, per F52.1's scope).

### The daily / local-day-boundary layer (pass 62 — the retention surface keyed on the player's own calendar day: streak + safety nets, weekly contract, equation-of-the-day, idle reminder, and the day-boundary math they share, written 2026-09-16)

Audited the local-day retention cluster end to end for the first time — pass 17 owned the offline-haul formulas, pass 40 inventoried every clock in the repo, and pass 56 covered the ad economy's day caps, but nobody had audited the *day-boundary math* itself: the seam where an epoch-millisecond timestamp becomes a `yyyy-MM-dd` local day key, and the −24 h / −N×24 h arithmetic that decides whether a claim is consecutive, a gap, or a reset. Surface: `dailyBonus.ts` (the streak ladder + the three safety nets — grace, freeze, repair — and the `getLocalDayKey`/`isYesterdayLocal`/`isTwoDaysAgoLocal` primitives the whole cluster imports), `weeklyChallenge.ts` (the Monday-keyed contract), `dailyEquation.ts` (the deterministic equation of the day), `idleReminder.ts`, the three persisting hooks (`useDailyBonus`, `useWeeklyChallenge`, `useDailyEquation`), and a repo-wide grep for every 24 h epoch computation.

**Verdict: the claim lifecycles are as defensive as any surface in the repo (pure derived claim checks, single-flight via synchronous ref publish, state outside the save, bounded-and-free safety nets — guardrails 1 and 3 hold) — and the layer's one real defect is the shared seam: the day-boundary comparison itself was epoch arithmetic on a calendar question.**

+ **F62.1 `daily:dst-day-boundary-misclassification` (Tier 2 — FIXED in pass 62, 2026-09-16):** `isYesterdayLocal`/`isTwoDaysAgoLocal` answered "was the last claim on the previous local day?" by comparing `getLocalDayKey(now − 24 h)` to the stored key (the 30-day grace/repair window edges used the same `now − N × 24 h` shape). A local day is 24 h most of the year — but not on the two DST transition days: on a 23 h day (spring forward) and a 25 h day (fall back), the 24 h epoch offset lands on the **wrong calendar day** for opens near local midnight, and it misclassifies in **both** directions. Reproduced on real transitions (America/New_York): on the spring shape, a clean consecutive claim opened in the first hour of the day after a 23 h day reads as *neither* yesterday nor two-days-ago → a hard streak reset (or a repair snapshot) where the player should get their streak day — while a genuine one-day gap opened there reads as *consecutive* → a free streak day that burns no safety net. On the fall-back shape (opens in the last hour of the 25 h day), a clean consecutive claim reads as a one-day gap → grace consumed, or freeze burned, or a hard reset if both are spent — while a genuine one-day gap reads as neither → a hard reset where the grace should have bridged it. The safety nets exist precisely to absorb one missed day, so this is not a "rare edge": it is the streak system's core judgment being wrong on two days a year, and the wrong direction is the expensive one (a 30+ day streak losing its 250k/day milestone to a "missed" day the player never missed). Fix (`dailyBonus.ts`): a new exported `localDayKeyDaysAgo(now, days)` — pure calendar arithmetic, `new Date(y, m, d − days)` (`Date` normalizes the month/year/leap overflow) — now backing `isYesterdayLocal`, `isTwoDaysAgoLocal`, and both window edges (`graceAvailable`, `repairAvailable`); the doc on the old "DST-tolerant enough" comment is replaced by the exact claim. Nets (`dailyBonus.test.ts`, +5 tests, 33 → 38): calendar subtraction across month/year/leap boundaries; and — because Node cannot switch its runtime TZ after start — a transition-finder that scans real local midnights in the *running* timezone (a ≠24 h midnight span = a transition day), so the two shape blocks exercise whatever DST regime the suite runs in and skip themselves cleanly in a no-transition zone (e.g. UTC CI). The 23 h block nets the consecutive + one-day-gap claims at the open of D+1 (the old math: hard reset / a free day); the 25 h block nets the same pair in the last hour of the 25 h day (the old math: grace burn / hard reset).

Recorded, not defects: **`weeklyChallenge.getLocalWeekKey` was already calendar-field math** (Monday midnight computed from the date fields, documented DST-safe) — the week boundary is a midnight, so no epoch subtraction is ever involved; the F62.1 fix brings the day-key side in line with the pattern the week side already used. **`dailyEquation` has no arithmetic at all** — identity is pure key equality (`solvedDay === getLocalDayKey(now)`), and a 23 h/25 h day is still one day key. **`analytics.ts` D1/D7 retention windows are epoch 2×/8×24 h by explicit design** — "local-day boundaries plus a generous window keep the metric honest across DST and timezone shifts … good enough to compare cohorts, not to bill anyone" (the code says so); it is a cohort metric, not a player-facing grant. **`idleReminder`'s 60 s threshold is a wall-clock interval, not day-keyed.** **The claim race is guarded** — both hooks publish the new state to `stateRef` synchronously before `setState` (the Android double-claim guard, commented at the site), the claim math always takes a fresh `Date.now()`, and the midnight/Monday rollover is a 60 s no-op poll that React bails on when the key is unchanged (the 0-miner fully-idle session). **The balance invariants hold** — the day-7 milestone (250k) is pinned by test to exceed days 1–6 combined (210k), and all three safety nets are bounded, free, earned-or-automatic, and derived (grace: one per rolling 30 days; freezes: one per 7 streak days, stock ≤ 3; repair: one per rolling 30 days, one-next-day-only window, only after losing ≥ 3 days) — no shield economy, no purchase path. **The weekly contract is cheat-shaped the same way as `goals.ts`** — deltas on monotonic lifetime metrics clamped at 0, re-snapshotted on roll, one claim per week, and `applyWeeklyClaim` is defensive like `applyDailyClaim` (never double-pays, never fabricates from null); an imported lower save can only cost progress, never grant it.

Method: the layer's shape is the day-key seam, so the four day-keyed modules and their three persisting hooks were read in full (`dailyBonus.ts` — the ladder, the three nets, the claim/apply pair, and the day primitives; `weeklyChallenge.ts` — the week key, baselines, deltas, claim; `dailyEquation.ts` — the seeded equation + key equality; `idleReminder.ts` — the interval decision; `useDailyBonus`/`useWeeklyChallenge`/`useDailyEquation` — the storage, the ref publish, the rollover poll); a repo grep for `24 * 60 * 60 * 1000` / `DAY_MS` / `86400000` isolated every 24 h epoch computation (only `dailyBonus.ts` day-boundary logic and the documented `analytics.ts` constants); a probe script replicating the old `isYesterdayLocal`/`isTwoDaysAgoLocal` against the real 2026-03-08 (23 h) and 2025-11-02 (25 h) transitions in America/New_York confirmed both misclassification directions before the fix; the runtime-TZ check (Node cannot switch TZ after start) is why the net finds real transition days instead of pinning a fixed one. `pnpm jest src/mines_of_doom/__test__/dailyBonus.test.ts` (38/38 green, including both transition shapes on this host). Gates: typecheck + lint clean; full jest in the fold commit.

### The purchase / spend transaction layer (pass 63 — the currency→upgrade paths: cost curves, the per-line affordability guards, and the buy-all planner/applier; written 2026-09-16)

Audited the purchase transaction machinery end to end for the first time — passes 8/19/25 owned the individual curves' balance, pass 37 the numeric ledger, and pass 29 the IAP catalog, but nobody had audited the deduction/guard surface itself: the eight cost curves as a set, the nine `useGameEngine` purchase functions (single-level + buy-all appliers), the `PurchaseButtons` disabled flags they mirror, `hasAffordablePurchase`, and the `computeBuyAll` greedy planner.

**Verdict: the machinery is coherent — every currency movement is guarded at the UI, the buy-all planner is exact (never overruns the integer budget, caps and goal-tier gates respected, conversion excluded), and the guard ledger (`totalGemsSpent`/`totalGemsMinted`) is updated on every gem movement — and the layer's one real defect is a defense-in-depth asymmetry: the three legacy purchase functions had no engine-level affordability guard at all, so the UI disabled flag was the *only* thing standing between a future caller and a negative balance.**

+ **F63.1 `purchases:legacy-lines-unguarded` (Tier 3 — FIXED in pass 63, 2026-09-16):** `upgradePower`, `buyMiner`, and `buyGem` — the three oldest lines — deducted unconditionally inside their `setGameState` updaters, while every line added afterwards (`buyFastMiner`, `buyLegendaryMiner`, `buyGemChance`, `upgradeMinerPower`, `buyClickBoost`, `buyComboResist`) re-checks affordability inside the updater and no-ops when short (each commented "Affordability-guarded like the other purchases"). The UI's `disabled` flags mirror the exact same affordability test for all nine lines, so under normal operation the trio was safe — but the asymmetry meant any future caller (a hotkey, an auto-buy feature, a buy-all refactor) hitting the three legacy callbacks directly could drive `minerals`/`gems` negative, a state the rest of the economy (offline math, gem math, the ledger) never expects to see. Fix (`useGameEngine.ts`): the same one-line `if (n.minerals < BigInt(cost)) return n;` / `if (n.gems < cost) return n;` guard added to all three, matching the sibling pattern exactly; zero behavior change for the UI path (the button is disabled there first). Nets (`useGameEngine.test.ts`, the guard test rewritten to "every purchase is affordability-guarded"): all five wallet-draining single-level purchases now no-op at zero balance (the trio included) and land exactly at zero balance when funded (the `buyGem`-within-the-batch case pinned: the minted gem survives, `totalGemsMinted` counts grant + conversion, `totalGemsSpent` counts only the three miner lines).

Recorded, not defects: **the level-0 free upgrades are onboarding, not a curve bug** — `getMinerPowerUpgradeCost(0) = 0` is the Prospector's-License first-upgrade freebie, and `clickPower` starts at 1 (so the first *paid* click upgrade costs `1⁴ = 1` mineral); all other lines cost ≥ 1 from count 0. **Strict monotonicity holds on all eight curves** through 1000 consecutive levels and 200 random probes up to 10⁶ per curve (`purchases.test.ts`, new suite) — no plateau level, no inversion, so the buy-all greedy's cheapest-line selection can never cycle. **`computeBuyAll` is exact where it matters**: its float `totalCost` is display-only by contract, the engine re-checks every level with integer arithmetic at apply time, and the planner was netted on 400 random (minerals, gems) states to both re-derive the exact per-level sum (`totalCost === Σ cost(level+i)`) and never overrun the integer budget. **Caps and gates hold under unlimited budget**: the capped lines (`GEM_CHANCE_MAX_LEVELS`, `CLICK_BOOST_MAX_LEVELS`, `COMBO_RESIST_MAX_LEVELS`) are respected, and — the `showAllPurchases` case that makes the whole exercise necessary — `eligible()` gates every line on its goal-tier unlock *independently of visibility*, so a locked line is never in a plan even when the setting forces all buttons visible (netted). **The minerals group excludes the buy-a-gem conversion by construction** — buy-all must never drain the mineral wallet into gems. **The indicator dot shares the same predicate** — `hasAffordablePurchase` re-encodes each line's button disabled flag (visibility ∧ unlock ∧ affordability ∧ cap) in pure form, so a dot can never point at a disabled row. **React StrictMode double-invocation is safe by shape** — every updater is a pure function of `n` (guards included), so a double-fired updater recomputes the same result from the same state instead of double-spending.

Method: read the nine purchase functions + buy-all appliers in `useGameEngine.ts`, all eight cost curves + `computeBuyAll` + `hasAffordablePurchase` in `game.ts`, and the full `PurchaseButtons.tsx` disabled-flag surface (the `showAllPurchases` branch in `MinesOfDoom.tsx` is what made the planner-side gate check load-bearing); netted with the new `purchases.test.ts` (9 tests: curve monotonicity/positivity, game-start cost states, `computeBuyAll` exactness + budget, caps, unlock gates, conversion exclusion) and the rewritten guard test in `useGameEngine.test.ts`. Gates: typecheck + lint clean, full jest 1257/1257.

### The web-e2e stub surface (pass 65 — the hermetic network layer the web e2e suite trusts: what the stubs promise vs. what the real backends do, written 2026-09-16)

Audited the last named candidate from the pass-64 close-out: the client-side stubs that make the web e2e hermetic (`e2e/web/stubs.ts` — the three install functions that mirror the app's three external integrations: the Ad Placement API loader, the Pocketbase sidecar + Stripe IAP path, and the GSI sign-in leg) plus the e2e static server (`server.mjs`, the test-mode flag injection) and the two provider/sidecar contracts they mirror (`adSenseProvider.web.ts`, `iapProvider.web.ts`, `pb_hooks` verify/identity paths). The question this layer answers: **when the web e2e goes green, is it green because the app is right, or because the stub is lenient where the real backend would bite?**

**Verdict: the stubs are faithful where it matters — the GSI stub verifies RS256 signatures + iss/aud/exp/iat exactly like the sidecar's claims path (a genuinely signed JWT crosses the wire), the IAP stub's `restore` returns storeIds through the app's own allowlist mapping, the ad stub implements the full push contract (`beforeReward` → `beforeAd` → `adViewed` → `afterAd`) with the grant keyed on `adViewed` only — and the layer had two leniency holes, both fixed.**

+ **F65.1 `e2e:iap-verify-stub-mints-any-token` (Tier 3 — FIXED in pass 65, 2026-09-16):** the IAP stub's `/api/app/verify` minted the entitlement for **any** string token, while the real sidecar's contract (pass 55: the entitlement is minted only for a session the sidecar can confirm with the Stripe API; an unconfirmed session is a 4xx refusal) means a wrong/stale/cross-product `iap_sid` is a *refusal the app must keep queued forever*. The lenient stub could never surface exactly that regression class — pass 58's batch-restore change is precisely the kind of edit that could swap a session id and still e2e-green. Fix (`stubs.ts`): the checkout route now records every issued session id per product (`issued: Map<string, Set<string>>`), and the verify route mints **only** for a token its own checkout issued for the same product id; anything else is a 400 + a `rejectedVerifies` record. The `restore` route is unchanged (returns the minted storeIds — the app's allowlist mapping is the net there). Net (`iap.spec.ts`, new test "a verify the sidecar can't confirm is refused and stays queued"): a forged entry is seeded into the bare AsyncStorage web key (`iapPendingVerifies`, the web provider's `PENDING_VERIFY_KEY`) via `addInitScript` before boot; boot leaves it untouched (no launch reconcile on web — the first IAP traffic is the purchase); the real `purchase()` replays the queue first, so the forged verify fires and is refused (400, recorded), the hosted checkout still issues a real session, the return-visit verify (an *issued* token) is accepted and mints, and the assertion triad holds: the forged entry is in `rejectedVerifies`, it **survives** in the pending queue (the pass-58 "failed verifies are never dropped" semantics) while the verified issued pair is gone from it, and the pack still shows owned. The existing round-trip test's `token === lastSession.sessionId` assertion was already the positive net; the forged-entry test is the negative one.
+ **F65.2 `e2e:live-ad-guard-pagead-path-hole` (Tier 3 — FIXED in pass 65, 2026-09-16):** the live-ad guard's regex treated `googlesyndication.com` as live traffic **only** on the `/pagead/lds` image path — every other ad-infrastructure path on the domain (`/pagead/1ps.js`, `showads.js`, `creativetags.js`, …) fell through `route.fallback()` and hit the **live network unrecorded**, so the test-mode test's "ZERO live ad requests" assertion (`guard.abortedLive === []`) was only as strong as the regex's path list. Fix (`stubs.ts`): `googlesyndication.com` is now live in its entirety — the loader script (`pagead/js/adsbygoogle.js`) is the single exempted resource, and both callers already exempt it *before* the live test runs (the stub-loader run intercepts the loader itself; `installLiveAdGuard` now explicitly falls the loader through, since the real loader must reach Google in test mode). The domain set (`doubleclick.net`, `googleadservices.com`, `adservice.google`, `adsystem.google`) is unchanged. Verified against the REAL Google loader in documented test mode (`ads.spec.ts` "real loader, Google test mode" — zero aborts, so the wider guard is not over-broad: the test-mode loader makes no other googlesyndication requests).
+ Clean (recorded, not defects): **the `PRODUCT_STORE_ID` mirror is a manual mirror of `PACK_SPECS` by design** — `stubs.ts` cannot import `iaps.ts` (it pulls `react-native` + the pixel-art sprite module into the Playwright process), so the 26-entry map is hand-kept; a programmatic diff confirms 26/26 in sync with zero value mismatches, and drift would surface as a *loud* e2e failure (the checkout stub 400s an unknown product id; the owned-check fails on a storeId mismatch) rather than a silent pass — acceptable, comment already says "keep in sync". **The ad stub is happy-path by construction** — it always fills and always reaches `adViewed`; the `adDismissed` (early-close) path is jest-netted in `adSenseProvider.test.ts` instead (two dedicated tests), which is the right division: deterministic e2e for the grant path, unit nets for the terminal-callback matrix. **The GSI stub deliberately omits `google.accounts.oauth2`** so a regression to the token API throws instead of minting an opaque token — a leniency *removed*, not added. **`server.mjs` is clean** — the test-mode flag injection matches the loader tag regardless of attribute order, the SPA fallback is traversal-guarded (`normalize` + `DIST` prefix check), the ping endpoint is the only non-static route; the one smell (an imported-but-unused `readFileSync` voided with a comment) was removed. **The `bodyOf` JSON parse is defensive** (non-JSON body → `{}` → the 400 path), matching the sidecar's fail-closed body handling.

Observation (not a pass-65 defect, no net): one unattributed jest flake during the pass's gate runs — 1 of 5 full-suite runs failed a single test, the other four (including the immediate re-run) were 1268/1268 clean, and the pass touched no file inside jest's `testMatch` (only `e2e/web/*`, which are `.spec.ts`, outside `**/*.test.[jt]s?(x)`). The failing test's name was not captured (the first run's output scrolled past); if it recurs, bisect with `--verbose` on a failing run. Time-dependent suspects (fake-timer suites, the DST transition-finder) are the likely class, but that is a guess, not an attribution.

Method: read all three stub install functions + `server.mjs` in full; read the two mirrored providers (`adSenseProvider.web.ts` in full — the push-contract shape the ad stub must satisfy; `iapProvider.web.ts` in full — the checkout/verify/restore request shapes, the pending-queue semantics, the storeId allowlist) and cross-checked the stub's response contracts against them (`verify` → 2xx-or-not drives the queue drop/keep; `restore` → `entitlements` storeId array through `IAP_STORE_IDS`); read `pb_hooks/storeVerify.js` (the fail-closed sidecar verify contract) and the pb_hooks identity path (the claims checks the GSI stub mirrors); diffed `PRODUCT_STORE_ID` against `PACK_SPECS` programmatically (26/26, no drift); checked `adDismissed` coverage in the jest suite. Gates: typecheck + lint clean; jest 1268/1268 (the flake noted above); **web e2e 11/11 passed (+1 live-GSI skip by design)** — including the new forged-token net, the widened-guard test-mode run against the real Google loader, and the pre-existing round-trip/signin/boot/ads suites.

### The message / toast surface (pass 66 — every path that puts a transient line of text in front of the player: the single toast slot, its 21 call sites, and the one a11y hole, written 2026-09-16)

Audited the message layer end to end: `useMessages.ts` (the state), its single instantiation (MinesOfDoom L188), every `displayMessage` call site (21 in `MinesOfDoom.tsx` plus the hook-level emitters that receive the callback as a prop: `useGameEngine`, `useDailyEquation`, `useGemPocket`, `useSettings`, `useIap`, `useCloudSave`, `useAdRewards`, `useDailyBonus`, `useIdleReminder`, `useWeeklyChallenge`), and the render site (the `messageOverlay` View + `styles.messageOverlay`/`messageText`). The question this layer answers: **can one message be lost, doubled, or left stranded — and can every player, including screen-reader users, hear it?**

**Verdict: the slot is sound — one instance, one timer, no races — but the one message surface the game has was silently invisible to screen readers (no `accessibilityLiveRegion` anywhere in `src`), which is fixed in this pass.**

+ **F66.1 `toast:not-a-live-region` (Tier 3 — FIXED in pass 66, 2026-09-16):** the toast overlay was conditionally mounted (`{showMessage && <View …>}`) and carried no a11y role — a toast that mounts *and unmounts with* its content is not announced by screen readers (a live region must already exist when its content changes), and a grep confirms `accessibilityLiveRegion` appears nowhere else in `src`, so no other surface picks the toasts up. Every transient the player's game communicates — combo up, depth reached, save code imported, the daily bonus, IAP results — was silent for the assistive-tech player. Fix (MinesOfDoom render site): the overlay `View` stays **permanently mounted** with `accessibilityLiveRegion="polite"` and its pre-existing `pointerEvents="none"` (it never intercepts input — the empty View has no background and the `messageOverlay` style is `position: absolute` with no size, so mounting it always has no layout or visual cost), and only the inner `Text` is now conditional on `showMessage`. On web, RNW maps `accessibilityLiveRegion` → `aria-live` (verified against the installed `react-native-web` build), so the same prop is the web story; natively it drives the platform announce. No i18n strings touched (no new text, the announcement reads the existing `toast.*` strings).
+ Clean (recorded, not defects): **single slot, no contention** — exactly one `useMessages()` exists (MinesOfDoom L188); the ten hook-level emitters take `displayMessage` as a prop, so ten subsystems share one queue instead of racing ten overlays. **No timer strands** — `displayMessage` clears the previous timeout before setting the next, so a rapid second message (a depth pass + combo up in the same swing) correctly supersedes the first and exactly one clear-pending timer exists at all times; there is no queue buffer by design (the last message wins), which is the right trade for a 2–5 s transient — a backlog of toasts would be worse than a dropped one. **All text is i18n** — every call site goes through `t("toast.*")`/`t("settings.*")` keys present in both `en.ts` and `es.ts` (typechecked against `en.ts`). **Timeouts are sane and uniform** — 2000–5000 ms across the surface, nothing under a second, nothing left up indefinitely. **`pointerEvents="none"`** was already correct and is preserved, so the always-mounted region cannot swallow a tap aimed at the canvas behind it.

Method: read `useMessages.ts` in full; enumerated and read every `displayMessage` call site in `MinesOfDoom.tsx` (21) and the prop plumbing in the ten emitting hooks (signature check, no per-hook instances); read the render site and `styles.messageOverlay`/`messageText`; grepped all of `src` for `accessibilityLiveRegion` (zero pre-existing uses); verified the RNW `accessibilityLiveRegion` → `aria-live` mapping in the installed build. Gates: typecheck + lint clean; jest 1273/1273.
