...
**Mines of Doom** (package name `minesofdoom`) is an idle/clicker mining game built with **Expo SDK 57 (React Native 0.86, new architecture)** and **expo-router**. It runs on web (deployed as a static site via GitHub Pages), Android, and iOS.
...
| `pnpm run deploy` | Export static web build to `dist/` and push via `gh-pages` (`predeploy` runs `expo export -p web`) |
