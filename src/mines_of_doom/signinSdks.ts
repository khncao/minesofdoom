 ...
 *     Google Identity Services (the gsi/client script + the ID client —
 *     `id.initialize({ client_id, callback })` then `id.prompt()` opens the
 *     consent popup, and `credential_response.credential` IS the signed
 *     idToken JWT; the sidecar verifies it exactly like the native one:
 *     RS256 + JWKS + iss + aud). The GSI token/authorization API
 *     (`google.accounts.oauth2.initTokenClient`) is deliberately NOT used
 *     here: Google documents it as data-access only ("not for sign-in"),
 *     and in production its `resp.access_token` was an opaque OAuth token —
 *     not a JWT — so the sidecar refused it ("malformed google token",
 *     401 on /api/app/auth/google).
 ...
 * kinds list is a pure function of the platform, and the SDK modules are only
 ...
 *  mechanism shipped with the build for this OS": Google rides the RN/Expo
 *  module on android + ios and the GSI script on web, Apple's Sign in
 *  with Apple on ios (web has no Apple button: it needs a domain-verified
 *  service id that does not exist yet). */
...
export const availableProviderKinds: ProviderKind[] = providerKindsForPlatform(
 Platform.OS,
);
...
export async function mintIdToken(kind: ProviderKind): Promise<string> {
 if (kind === "google") {
  return Platform.OS === "web" ? mintGoogleIdTokenWeb() : mintGoogleIdToken();
 }
...
/** The slice of the GSI ID client this module uses (sign-in only).
 *  `initialize()` registers the callback that delivers
 *  `credential_response.credential` — the SIGNED idToken JWT (RS256, whose
 *  `aud` is the web client id above — the same audience the sidecar's
 *  GOOGLE_CLIENT_ID pins; the server verifies it exactly like the native
 *  idToken). `prompt()` opens the consent popup and returns nothing: the
 *  outcome arrives through the registered callbacks. */
...
interface GsiIdApi {
 initialize(opts: {
  client_id: string;
  callback: (resp: { credential?: unknown }) => void;
  error_callback?: (err: unknown) => void;
 }): GsiIdClient;
 /** Documented call site (`google.accounts.id.prompt()`); `initialize()`
  *  also returns the client, but the namespace method is the API. */
 prompt(): void;
}
 ...
 * per page), then run the ID-client popup flow. Resolves the JWT idToken;
 * rejects `SignInCancelledError` when the user closes the popup (GSI's
 * documented `popup_closed*` errors — string or `{ type }` object, both
 * handled below) and a plain Error when the flow returns nothing usable
 * (the UI's single inline error).
 */
export async function mintGoogleIdTokenWeb(): Promise<string> {
 ...
 let id = w.google?.accounts?.id;
 if (!id) {
  await loadGsiScript(w);
  id = w.google?.accounts?.id;
  if (!id) {
  ...
  id.initialize({
   ...
   callback: (resp) => {
    ...
    if (typeof credential === "string" && credential.length > 0) {
     ...
     settle(() => reject(new Error("google web sign-in returned no id token")));
    }
   },
   error_callback: (err) => {
    const code = gsiErrorCode(err);
    if (code.includes("closed")) {
     settle(() => reject(new SignInCancelledError("google")));
    } else {
     settle(() =>
      reject(new Error(`google web sign-in failed: ${code || String(err)}`)),
     );
    }
   },
  });
...
/** GSI's error_callback has shipped both shapes over its lifetime — the
 *  legacy string code (`"popup_closed_by_user"`) and the documented
 *  `{ type, message }` object (`type: "popup_closed"` /
 *  `"popup_failed_to_open"`). Reduce either to the code string; the caller
 *  pattern-matches it. */
function gsiErrorCode(err: unknown): string {
 ...
 *  "not injected yet → inject it ourselves". (The same script exposes the
 *  `google.accounts.id` and `google.accounts.oauth2` namespaces; only the
 *  id client is used here — the module header explains why the token API
 *  is off the table for sign-in.)
 */
